...
Block* allocate(int size) {
    ...
    if (!bestFit) return NULL;  // No suitable block found
...
void mergeBuddies(Block* block) {
    while (block->size != MAX_MEM_SIZE) {
        ...
        if (!buddy) break; // No buddy found or buddy is not free
...
void printMemoryContents() {
    ...
    for (current = memory; current != NULL; current = current->next) {
        printf("%d\t%s\t%d\n", current->size, current->isFree ? "Free" : "Allocated", current->memoryLocation);
    }
    printf("\n");
}
