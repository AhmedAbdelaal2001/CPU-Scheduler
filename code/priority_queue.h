...
void minHeapify(PriorityQueue* pq, int i) {
    ...
    if (l < pq->size && pq->processArray[l]->priority < pq->processArray[smallest]->priority) smallest = l;
    if (r < pq->size && pq->processArray[r]->priority < pq->processArray[smallest]->priority) smallest = r;
...
void heapDecreaseKey(PriorityQueue* pq, int i, int key) {
    if (key > pq->processArray[i]->priority) return;
...
void minHeapInsert(PriorityQueue* pq, struct process* newProcess) {
    if (pq->size == MAX_PROCESSES) return;
    pq->size++;
